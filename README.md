...
- 🎨 **Красивая консоль** — ASCII-заставка, крупный шрифт, центрированное окно, прокрутка истории
...
|---|---|
| ОС | Windows 10 / 11 |
| Игра | PEAK (Steam) с установленным [BepInEx](https://thunderstore.io/c/peak/) |
| Интернет | доступ к `thunderstore.io` |
| Права | запись в папку игры (обычно достаточно обычного пользователя) |

## 📦 Установка

### Способ 1: готовый .exe (проще)
...
1. Запустите программу — появится заставка с прогресс-баром
2. Программа скачает каталог ThunderStore и просканирует ваши моды
3. Появится отчёт:
   ```
   --- РЕЗУЛЬТАТ ПРОВЕРКИ ---
     [OK]        PEAKLib.Core 1.7.2
     [UPDATE]    EaglesEye: 3.0.1  ->  3.0.2
     [NOT FOUND] MyLocalMod 0.1 (нет на ThunderStore, пропущен)
   ```
